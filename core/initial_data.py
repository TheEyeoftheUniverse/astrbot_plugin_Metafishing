...
FISH_DATA = [
    # Format: (name, description, rarity, base_value, icon_url)
    # Rarity 1
    ("小鲫鱼", "一条非常常见的小鱼。", 1, 10, None),
    ("泥鳅", "滑溜溜的小家伙。", 1, 15, None),
    ("小虾", "常见的水底生物。", 1, 20, None),
    ("沙丁鱼", "成群结队的小型海鱼。", 1, 12, None),
    ("小黄鱼", "体色金黄，常见食用鱼。", 1, 18, None),
    ("海虾", "比淡水虾稍大。", 1, 25, None),
    ("破旧的靴子", "谁把靴子扔水里了？还能穿吗？", 1, 1, None),
    ("生锈的铁罐", "被水泡得不成样子了。", 1, 2, None),
    ("缠绕的水草", "一团湿漉漉的水生植物。", 1, 1, None),
    ("普通石头", "就是一块普通的石头，沉甸甸的。", 1, 0, None),
    ("罗非鱼", "繁殖能力很强，适应性好。", 1, 13, None),
    ("鲦鱼", "溪流中常见的小型杂食鱼。", 1, 8, None),
    ("鮈鱼", "生活在水底的小型鲤科鱼。", 1, 9, None),
    ("银鲈", "沿海常见的小型银色鱼类。", 1, 16, None),
    ("大西洋黄花鱼", "能发出类似蛙鸣的鼓噪声。", 1, 17, None),
    ("斑点鱼", "尾鳍基部有个明显的黑斑。", 1, 14, None),
    ("食蚊鱼", "以蚊子幼虫为食，体型微小。", 1, 5, None),
    ("凤尾鱼", "常被用来制作鱼干或罐头。", 1, 7, None),
    ("胡瓜鱼", "体型细长，有一种特殊的黄瓜味。", 1, 11, None),

    # Rarity 2
    ("草鱼", "喜欢吃草的淡水鱼。", 2, 50, None),
    ("鲤鱼", "据说能带来好运。", 2, 60, None),
    ("鲢鱼", "头部较大，滤食性鱼类。", 2, 55, None),
    ("鳊鱼", "体型侧扁，味道不错。", 2, 65, None),
    ("鲅鱼", "体型较长，常做熏鱼。", 2, 70, None),
    ("带鱼", "身体扁长如带。", 2, 75, None),
    ("黄花鱼", "分为大黄鱼和小黄鱼。", 2, 80, None),
    ("鲳鱼", "肉厚刺少。", 2, 85, None),
    ("河豚", "生气时会鼓成一个球，可能有毒！", 2, 40, None),
    ("小丑鱼", "和海葵共生，颜色鲜艳。", 2, 45, None),
    ("螃蟹", "横行霸道的甲壳生物。", 2, 35, None),
    ("河鲈", "广泛分布的淡水掠食性鱼类。", 2, 48, None),
    ("蓝鳃太阳鱼", "北美常见的太阳鱼，色彩鲜艳。", 2, 42, None),
    ("莓鲈", "北美常见的两种太阳鱼，分黑莓鲈和白莓鲈。", 2, 44, None),
    ("玻璃梭鲈", "眼睛在光下会反光，夜行性。", 2, 68, None),
    ("牛头鱼", "头大嘴宽，常见的小型鲶鱼。", 2, 38, None),
    ("羊头鱼", "牙齿像羊齿，能咬碎贝壳。", 2, 72, None),
    ("比目鱼", "眼睛长在同一侧，身体扁平。", 2, 62, None),
    ("黑线鳕", "背部有明显的黑色侧线。", 2, 58, None),
    ("青鳕", "常见的食用鱼，肉质白色。", 2, 52, None),
    ("无须鳕", "深水鱼类，常用于鱼排。", 2, 54, None),
    ("鲭鱼", "体型纺锤，游泳速度快。", 2, 67, None),
    ("斧头鱼", "身体侧扁像斧头，深海发光鱼。", 2, 88, None),
    ("鳞鲀", "有一个可以锁定的背鳍棘刺。", 2, 78, None),

    # Rarity 3
    ("鲈鱼", "肉质鲜美，适合清蒸。", 3, 100, None),
    ("黑鱼", "生性凶猛的掠食者。", 3, 150, None),
    ("鳜鱼", "又称桂鱼，名贵食用鱼。", 3, 120, None),
    ("胭脂鱼", "色彩艳丽，国家保护动物(游戏中)。", 3, 180, None),
    ("鲨鱼", "海洋中的顶级掠食者之一。", 3, 200, None),
    ("金枪鱼", "高速游泳的大型鱼类。", 3, 250, None),
    ("石斑鱼", "名贵的海产鱼类。", 3, 300, None),
    ("鲷鱼", "体色鲜艳，寓意吉祥。", 3, 350, None),
    ("食人鱼", "牙齿锋利，成群时很危险。", 3, 110, None),
    ("章鱼", "非常聪明，有很多触手。", 3, 140, None),
    ("狗鱼", "体型细长，嘴巴像鸭嘴。", 3, 130, None),
    ("大口黑鲈", "北美最受欢迎的游钓鱼之一。", 3, 145, None),
    ("鳟鱼", "生活在清澈冷水中，种类繁多。", 3, 165, None),
    ("鲑鱼", "会洄游产卵的著名鱼类。", 3, 175, None),
    ("鲟鱼", "古老的鱼类，产鱼子酱。", 3, 280, None),
    ("匙吻鲟", "吻部像船桨，滤食性。", 3, 260, None),
    ("鲶鱼", "有胡须，种类繁多，从大到小。", 3, 115, None),
    ("鲯鳅", "体色鲜艳，游泳速度快。", 3, 290, None),
    ("军曹鱼", "体型像鲨鱼，常跟随大型海洋生物。", 3, 240, None),
    ("梭鱼", "体型细长，牙齿锋利的海中猎手。", 3, 210, None),
    ("鮟鱇鱼", "长相奇特，用诱饵捕食。", 3, 190, None),
    ("灯笼鱼", "深海中最常见的发光鱼类。", 3, 95, None),
    ("蝰鱼", "牙齿极长，无法完全闭合嘴巴。", 3, 230, None),

    # Rarity 4
    ("金龙鱼", "价值不菲的观赏鱼。", 4, 500, None),
    ("清道夫", "常被误认为能清洁水质。", 4, 450, None),
    ("娃娃鱼", "学名大鲵，叫声似婴儿啼哭。", 4, 800, None),
    ("蓝鳍金枪鱼", "金枪鱼中的极品，价格昂贵。", 4, 1000, None),
    ("剑鱼", "上颌延长呈剑状。", 4, 1200, None),
    ("海豚", "聪明友善的海洋哺乳动物。", 4, 900, None),
    ("鲸鱼", "巨大的海洋哺乳动物。", 4, 1500, None),
    ("电鳗", "能释放强大的电流。", 4, 600, None),
    ("海龟", "长寿的海洋爬行动物。", 4, 750, None),
    ("深海鮟鱇", '用头顶的"灯笼"诱捕猎物。', 4, 700, None),
    ("沉没的宝箱", "里面似乎装着什么东西？", 4, 100, None),
    ("腔棘鱼", "被称为活化石的古老鱼类。", 4, 1100, None),
    ("桔连鳍鲑", "寿命极长，生长缓慢的深海鱼。", 4, 850, None),
    ("皇带鱼", '世界上最长的硬骨鱼，传说中的"海蛇"。', 4, 1300, None),
    ("尖牙鱼", "牙齿相对于体型来说非常巨大。", 4, 680, None),
    ("水滴鱼", "离开深海高压环境后变成一滩凝胶状。", 4, 400, None),
    ("光颌鱼", "能发出和感知红光的深海鱼。", 4, 950, None),
    ("角鮟鱇", "雌雄体型差异巨大的深海鮟鱇。", 4, 820, None),
    ("黑魔鬼鱼", "另一种深海鮟鱇，通体黑色。", 4, 880, None),
    ("鳕鲈", "生活在海底峡谷边缘的鱼类。", 4, 720, None),
    ("大比目鱼", "大型的比目鱼，价值较高。", 4, 980, None),
    ("旗鱼", "背鳍高大如旗，速度极快。", 4, 1400, None),

    # Rarity 5
    ("锦鲤", "极其珍贵，象征吉祥。", 5, 1000, None),
    ("龙王", "传说中的四海之主(的投影?)。", 5, 5000, None),
    ("美人鱼", "传说中的海洋生物。", 5, 8000, None),
    ("深海巨妖", "盘踞在深渊中的恐怖存在。", 5, 10000, None),
    ("海神三叉戟", "遗落在某处的神器(的一部分)。", 5, 15000, None),
    ("时运沙漏", "据说是能一个能窥见未来片刻的神器。", 5, 20000, None),
    ("克拉肯之触", "传说巨妖的一小部分，依然充满力量。", 5, 11000, None),
    ("许德拉之鳞", "九头蛇的鳞片，闪耀着再生的光泽。", 5, 13000, None),
    ("尘世巨蟒之环", "环绕世界之蛇蜕下的一节，蕴含世界之力。", 5, 22000, None),
    ("神马", "神话中波塞冬的坐骑，半马半鱼。", 5, 7000, None),
    ("狮子鱼", "外表华丽但棘刺剧毒，属于入侵物种（游戏中设为稀有）。", 5, 800, None),
    ("幽灵鲨", "深海软骨鱼，样子奇特，像缝合怪。", 5, 6500, None),
    ("吸血鬼乌贼", "既不是乌贼也不是章鱼，能像斗篷一样包裹自己。", 5, 7500, None),
    ("阿斯皮多凯隆幼龟", "传说中的岛龟的幼崽，背上已有微缩景观。", 5, 16000, None),
    ("最深之鱼", "来自马里亚纳海沟深处的神秘生物，几乎无法承受低压。", 5, 30000, None),

    # Rarity 6 - 神话生物
    ("苍穹裂隙守护者", "守护着空间裂缝的神秘生物，身体半透明，散发着星辰之光。", 6, 50000, None),
    ("时间系蝠鲼", "游弋在时间与空间之间的巨大蝠鲼，翅膀上刻有古老符文。", 6, 75000, None),
    ("虚空鲸", "来自未知维度的巨鲸，能够吞噬光线和声音。", 6, 60000, None),
    ("星河龙鱼", "身体像银河般璀璨的龙鱼，传说它们的鳞片能预言未来。", 6, 45000, None),

    # Rarity 7 - 古神眷属
    ("深渊之眼", "克苏鲁神话中的古老存在，仅仅凝视就能让人发疯。", 7, 120000, None),
    ("原始之母", "诞生于太古时代的原始生命体，蕴含着创世的力量。", 7, 100000, None),
    ("混沌游鱼", "从混沌中诞生的鱼类，身体不断变化形态。", 7, 90000, None),

    # Rarity 8 - 宇宙奇点
    ("世界之鱼", "传说中承载着整个世界的巨鱼，它的一次摆尾能引起海啸。", 8, 500000, None),
    ("创世神鳞", "创造万物的神明遗留的鳞片，拥有无穷的创造力。", 8, 1000000, None),
]
...
BAIT_DATA = [
    ...
    # --- Rarity 5 (传说) ---
    ("珍馐诱饵", "蕴含着远古海潮气息，能吸引珍稀鱼类。", 5, "提高稀有鱼上钩率",
     0, 2500, 4, 0.0, 0.08, 0.0, 1.0, 1.0, True),
...
TITLE_DATA = [
    ...
    (15, "老练钓手", "累计钓鱼次数达到进阶目标", "{username}, 老练钓手!"),
    (16, "远航钓手", "累计钓鱼次数达到资深目标", "{username}, 远航钓手!"),
