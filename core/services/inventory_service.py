...
class InventoryService:
    ...
    def __init__(
        ...
        config: Dict[str, Any],
        expedition_service=None,  # 可选的科考服务依赖
    ):
    ...
    def _get_user_friendly_description(self, item_template) -> str:
        ...
        将技术性的效果描述转换为用户友好的描述
        """
        effect_desc = item_template.effect_description or ""
        ...
        # 否则提供通用描述
        effect_type = getattr(item_template, "effect_type", "") or ""
        effect_type_lower = effect_type.lower() if effect_type else ""
        
        if effect_type == "zone_pass":
            return "用于进入特定钓鱼区域的通行证"
        elif "BUFF" in effect_desc or "buff" in effect_type_lower:
            return "使用后可以获得临时属性加成"
        elif "COINS" in effect_desc:
            return "使用后可以获得金币奖励"
        elif "COOLDOWN" in effect_desc:
            return "使用后可以重置钓鱼冷却时间"
        elif "BAIT" in effect_type or "bait" in effect_type_lower:
            return "钓鱼时使用的鱼饵，可提升捕获效果"
        else:
            return getattr(item_template, "description", "特殊道具")

    def sell_fish(self, user_id: str, fish_id: int, quantity: int = 1, quality_level: int = 0) -> Dict[str, Any]:
        """
        向系统出售指定鱼类。
        ...
        # 计算价值（高品质鱼双倍价值）
        total_value = fish_template.base_value * quantity * (1 + quality_level)
        ...
        # 更新科考进度（如果用户在科考中）
        expedition_update = None
        if self.expedition_service:
            try:
                sold_fish = {fish_id: quantity}
                expedition_update = self.expedition_service.update_expedition_on_sell_fish(user_id, sold_fish)
            except Exception as e:
                pass

        quality_label = "✨高品质" if quality_level == 1 else "普通"
        message = f"💰 成功卖出 {quantity} 条{quality_label}{fish_template.name}，获得 {total_value} 金币"
        
        # 添加科考进度提示
        if expedition_update:
            message += "\n\n🔬 科考进度更新："
            for fish_name, info in expedition_update["targets"].items():
                message += f"\n  • {fish_name}: +{info['quantity']} 条（进度 {info['progress']}）"
            message += f"\n  📊 总进度：{expedition_update['total_progress']*100:.1f}%"
    ...
    def sell_all_fish(self, user_id: str, keep_one: bool = False) -> Dict[str, Any]:
        ...
        for item in fish_inventory:
            ...
            if fish_template:
                # 记录出售的鱼
                sold_fish[item.fish_id] = item.quantity
        ...
        # 更新科考进度（如果用户在科考中）
        expedition_update = None
        if self.expedition_service and sold_fish:
            try:
                expedition_update = self.expedition_service.update_expedition_on_sell_fish(user_id, sold_fish)
            except Exception as e:
                # 科考更新失败不影响出售流程
    ...
    def sell_fish_by_rarity(self, user_id: str, rarity: int) -> Dict[str, Any]:
        ...
        for item in fish_inventory:
            ...
            if fish_info and fish_info.rarity == rarity:
                # 记录出售的鱼
                sold_fish[fish_id] = item.quantity
    ...
    def sell_fish_by_rarities(self, user_id: str, rarities: list[int]) -> Dict[str, Any]:
        ...
        if expedition_update:
            message += "\n🔬 科考进度更新：\n"
            for fish_name, info in expedition_update["targets"].items():
                message += f"  • {fish_name}: +{info['quantity']} 条（进度 {info['progress']}）\n"
            message += f"  📊 总进度：{expedition_update['total_progress']*100:.1f}%"
    ...
    def use_bait(self, user_id: str, bait_id: int) -> Dict[str, Any]:
        ...
        # 检查鱼竿星级要求
        if bait_template.required_rod_rarity > 0:
            equipped_rod_instance = self.inventory_repo.get_user_equipped_rod(user_id)
            if not equipped_rod_instance:
                return {
                    "success": False, 
                    "message": f"该鱼饵需要装备至少 {bait_template.required_rod_rarity} 星的鱼竿才能使用"
                }
            
            rod_template = self.item_template_repo.get_rod_by_id(equipped_rod_instance.rod_id)
            if not rod_template or rod_template.rarity < bait_template.required_rod_rarity:
                return {
                    "success": False, 
                    "message": f"该鱼饵需要装备至少 {bait_template.required_rod_rarity} 星的鱼竿才能使用，当前鱼竿星级不足"
                }
