...
class UserService:
    ...
    def get_user_current_rod(self, user_id: str) -> Dict[str, Any]:
        """
        获取用户当前装备的鱼竿信息。
        """
        user = self.user_repo.get_by_id(user_id)
        if not user:
            return {"success": False, "message": "用户不存在"}
        current_rod = self.inventory_repo.get_user_equipped_rod(user_id)
        if not current_rod:
            return {"success": True, "rod": None}
        rod_template = self.item_template_repo.get_rod_by_id(current_rod.rod_id)
        if not rod_template:
            return {"success": False, "message": "鱼竿不存在"}
        return {
            "success": True,
            "rod": {
                "id": current_rod.rod_id,
