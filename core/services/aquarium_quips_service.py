...
# 预置兜底短评池（至少 12 条；填空式：可直接接在「{某人}凝视着{鱼名}，觉得____」之后）
...
class AquariumQuipsService:
    ...
    def _build_prompt(batch: List[Any]) -> str:
        lines = [
            ...
            "请为每条鱼生成 3 条 中文短句，每条 12-22 个汉字，作为别人游客看完展览后的「想法从句」。",
            "",
            "【格式硬要求】",
            "  · 每条短句必须能直接接在「{某人}凝视着{鱼名}，觉得____」后面读起来通顺。",
            "  · 短句应以「它」「这鱼」「这条鱼」「像」「仿佛」「好像」「一脸」等开头。",
            "  · 不要在短句里重复鱼名。",
        ...
        lines.extend([
            ...
            '{"103": ["它在缸里晃来晃去活像周一早晨的我", "它一脸不知道自己为啥还活着的样子", "它和缸里的水草达成了奇怪的默契"]}',
