...
## v4.0.0 (基线重建 + 静态数据收口 + 科考规则重构)

- **🗃️ 数据初始化重构**：删除历史迁移链，改为单一最新基线迁移，直接以最新 schema 与静态 seed 初始化数据库。
- **📈 交易所改为纯读库**：期货商品、初始价格与调价规则全部改由数据库静态表驱动，不再依赖代码兜底。
- **🧹 旧道具体系清理**：移除当前基线已无引用的旧道具效果代码、旧 `initial_data.py` 与相关无效入口。
- **🔬 科考规则更新**：
  - 取消科考时限相关判定与展示文案，不再出现超时拦截。
  - 探险级科考目标鱼仅从区域 `1-4` 抽取。
  - 征服级与圣域级科考目标鱼继续从全区域抽取。
- **📚 文档同步**：README 按当前实际玩法、命令和 WebUI 能力重写。
...
- **📦 插件元数据规范化**：优化插件元数据配置，提升插件管理体验
  - 插件名称规范化：`name` 字段改为 `astrbot_plugin_metafishing`
  - 新增 `display_name` 字段：当前显示名称为 `MetaFishing！`
