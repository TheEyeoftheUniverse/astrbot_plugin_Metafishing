...
async def shop(plugin: "FishingPlugin", event: AstrMessageEvent):
    ...
    for i, e in enumerate(items):
        ...
        # 库存显示：如果非无限库存，显示为 "剩余数量 (已购/总库存)"
        if item.get("stock_total") is None:
            stock_str = "无限"
        else:
            stock_total = item.get('stock_total', 0)
            stock_sold = item.get('stock_sold', 0)
            stock_remaining = stock_total - stock_sold
            stock_str = f"{stock_remaining} (已购{stock_sold}/{stock_total})"
            ...
            if reward["reward_type"] == "rod":
                ...
                if rod_template:
                    ...
                    if rod_template.bonus_fish_quality_modifier != 1.0:
                        ...
                        bonus_info.append(f"品质加成: {bonus_pct:+.1f}%")
                    if rod_template.bonus_fish_quantity_modifier != 1.0:
                        bonus_pct = (rod_template.bonus_fish_quantity_modifier - 1.0) * 100
                        bonus_info.append(f"数量加成: {bonus_pct:+.1f}%")
                    if rod_template.bonus_rare_fish_chance != 0.0:
                        bonus_info.append(f"稀有度加成: +{rod_template.bonus_rare_fish_chance:.1%}")
            elif reward["reward_type"] == "accessory":
                accessory_template = plugin.item_template_repo.get_accessory_by_id(
                    reward.get("reward_item_id")
                )
                if accessory_template:
                    if accessory_template.bonus_fish_quality_modifier != 1.0:
                        bonus_pct = (accessory_template.bonus_fish_quality_modifier - 1.0) * 100
                        bonus_info.append(f"品质加成: {bonus_pct:+.1f}%")
                    if accessory_template.bonus_fish_quantity_modifier != 1.0:
                        bonus_pct = (accessory_template.bonus_fish_quantity_modifier - 1.0) * 100
                        bonus_info.append(f"数量加成: {bonus_pct:+.1f}%")
                    if accessory_template.bonus_rare_fish_chance != 0.0:
                        bonus_info.append(f"稀有度加成: +{accessory_template.bonus_rare_fish_chance:.1%}")
                    if accessory_template.bonus_coin_modifier != 1.0:
                        bonus_pct = (accessory_template.bonus_coin_modifier - 1.0) * 100
                        bonus_info.append(f"金币加成: {bonus_pct:+.1f}%")
        
        if bonus_info:
            msg += f"├─ 加成: {' | '.join(bonus_info)}\n"
